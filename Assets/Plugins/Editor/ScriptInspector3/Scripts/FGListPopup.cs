...
public class FGListPopup : FGPopupWindow
{
	...
	class SymbolDefinitionComparer : IComparer<SymbolDefinition>
	{
		...
		{
			var c = string.Compare(NameOf(a), NameOf(b), true);
			if (c == 0)
				c = string.Compare(NameOf(a), NameOf(b));
			return c;
		}
	}
	...
	public string TypedInPart
	{
		...
		set
		{
			...
			else
			{
				...
				else
				{
					...
					else
					{
						...
						focusRecent = bestMatches.Count > 1 || currentItem < 0;
					}
				}
			}
			...
			if (focusRecent)
			{
				if (bestMatches != null && bestMatches.Count > 1)
				{
					for (var i = recentCompletions.Count; i-- > 0; )
					{
						if (bestMatches.Contains(recentCompletions[i]))
						{
							...
							currentItem = filteredData.BinarySearch(tempSymbol, new SymbolDefinitionComparer());
							break;
						}
					}
				}
				...
				{
					tempSymbol.name = recentCompletions[i];
					var recentItemIndex = filteredData.BinarySearch(tempSymbol, new SymbolDefinitionComparer());
	...
	private static void LoadSymbolIcons()
	{
		...
		var oneForAll = new HashSet<SymbolKind> { SymbolKind.Namespace, SymbolKind.EnumMember, SymbolKind.Parameter,
			...
			SymbolKind.TypeParameter, SymbolKind.Label, SymbolKind.LocalConstant, SymbolKind.Constructor };
		...
		for (var i = 0; i < kinds.Length; i++)
		{
			...
			//	Debug.Log("No icon for " + kind);
			//else
			//	Debug.Log("Loaded icon VSObject_" + kind + ".png");
			//if (symbolIcons[index, 1] == null)
			//	Debug.Log("No icon for protected " + kind);
			//else
			//	Debug.Log("Loaded icon VSObject_" + kind + "_Protected.png");
			//if (symbolIcons[index, 2] == null)
			//	Debug.Log("No icon for private " + kind);
			//else
			//	Debug.Log("Loaded icon VSObject_" + kind + "_Private.png");
		}
	...
	public void SetCompletionData(HashSet<SymbolDefinition> data)
	{
		...
		System.Array.Sort(this.data, new SymbolDefinitionComparer());
		UpdateTypedInPart();
	...
	private void OnGUI()
	{
		...
		GUI.Label(scrollViewRect, GUIContent.none);
		...
		GUI.Label(scrollViewRect, GUIContent.none, textEditor.styles.scrollViewStyle);
		...
		if (Event.current.type == EventType.Repaint)
		{
			...
			{
				var itemContent = new GUIContent(" <b>" + filteredData[i].GetName() + "</b>");
			...
			for (var i = Mathf.Min(filteredData.Count, scrollPosition + maxListItems + 1); --i >= scrollPosition; )
			{
				...
				var icon = symbolIcons[(int) symbol.kind, symbol.IsPublic | symbol.IsInternal ? 0 : symbol.IsProtected ? 1 : 2] ?? keywordIcon;
				...
				var itemContent = new GUIContent(' ' + styledName, icon);
				if (filteredData[i].GetTypeParameters() != null)
					itemContent.text += "<>";
	...
	public SymbolDefinition OnOwnerGUI()
	{
		...
		if (Event.current.type == EventType.keyDown)
		{
			if (!(Event.current.alt || Event.current.command || Event.current.control) &&
				currentItem >= 0 && ("\t\n {}[]().,:;+-*/%&|^!~=<>?@#\'\"\\".IndexOf(Event.current.character) >= 0 || Event.current.keyCode == KeyCode.KeypadEnter))
			{
