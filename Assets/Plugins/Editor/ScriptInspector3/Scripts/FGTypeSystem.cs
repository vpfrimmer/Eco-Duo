...
public abstract class Scope
{
	...
	public NamespaceScope EnclosingNamespaceScope()
	{
		...
		{
			var parentNamespaceScope = parentScope as NamespaceScope;
...
public abstract class TypeDefinitionBase : SymbolDefinition
{
	...
	public virtual bool CanConvertTo(TypeDefinitionBase otherType)
	{
		...
		if (this == builtInTypes_float &&
			otherType == builtInTypes_double)
				return true;
		//if (this == builtInTypes["double"])
		//	return false;
		//if (this == builtInTypes["decimal"])
		//	return false;
		if (this == builtInTypes_char &&
			otherType == builtInTypes_string)
				return true;
		//if (this == builtInTypes["string"])
		//	return false;
		//if (this == builtInTypes["bool"])
		//	return false;
...
//			return tooltipText;
	...
	public override bool CanConvertTo(TypeDefinitionBase otherType)
	{
		...
		if (asArrayType != null && rank == asArrayType.rank)
...
//		}
	...
	private bool checkingDerivesFromBase = false;
	public override bool DerivesFromRef(ref TypeDefinitionBase otherType)
	{
		if (otherType == null)
			return false;
			...
			if (argumentNode != null)
			{
				...
				if (argumentValueNode != null)
				{
					...
					if (resolvedArguments[i] != null)
						argumentTypes.Add(resolvedArguments[i].TypeOf() as TypeDefinitionBase);
	...
	public ConstructedMethodDefinition ConstructMethod(SymbolReference[] typeArgs)
	{
		...
		if (typeArgs != null)
		{
			unchecked // ignore overflow
			{
				hash = (int)2166136261;
				foreach (var arg in typeArgs)
					hash = hash * 16777619 ^ arg.definition.GetHashCode();
			}
		}
		...
		if (constructedMethods.TryGetValue(hash, out result))
		{
			if (result.IsValid() && result.typeArguments != null)
			{
				...
				for (var i = 0; i < numTypeArgs; ++i)
				{
					...
					if (definition == null || !definition.IsValid() || definition != typeArgs[i].definition)
					{
...
public class NamespaceScope : Scope
{
	...
	{
		var id = invokedSymbol != null && invokedSymbol.kind != SymbolKind.Error ? invokedSymbol.name : SymbolDefinition.DecodeId(invokedLeaf.token.text);
		...
		definition.CollectExtensionMethods(id, typeArgs, memberOf, extensionsMethods, context);
		...
		for (var i = importedNamespaces.Count; i --> 0; )
		{
			...
			if (nsDef != null)
				nsDef.CollectExtensionMethods(id, typeArgs, memberOf, extensionsMethods, context);
		}
		if (extensionsMethods.Count > 0)
		{
	...
	//	data.UnionWith(declaration.typeAliases.Keys);
		foreach (var i in declaration.importedNamespaces)
			i.definition.GetCompletionData(data, fromInstance, assembly);
...
//			return tooltipText;
	...
	protected string DebugValue()
	{
			...
			(kind == SymbolKind.Field || kind == SymbolKind.Property))
		{
			...
			if (!IsStatic)
			{
				if (typeof(UnityEngine.ScriptableObject).IsAssignableFrom(runtimeType))
				{
					const BindingFlags instanceMember = BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic;
					var allInstances = UnityEngine.Resources.FindObjectsOfTypeAll(runtimeType);
...
//					if (asMemberOf.kind != SymbolKind.Event)
			...
			case "relationalExpression":
				...
				for (var i = 2; i < node.numValidNodes; i += 2)
				{
					...
					{
						ResolveNode(node.ChildAt(i), scope);
						part = builtInTypes_bool.GetThisInstance(); // HACK
					}
				}
				return part;
...
public class AssemblyDefinition : SymbolDefinition
{
		...
		get {
			if (_domainAssemblies == null || _domainAssemblies.Length != AppDomain.CurrentDomain.GetAssemblies().Length)
			{
				...
				foreach (var assembly in _domainAssemblies)
				{
					...
					else if (path.EndsWith("/unityengine.dll")
						|| path.EndsWith("/unityeditor.dll")
						|| path.EndsWith("/system.dll")
						|| path.EndsWith("/system.core.dll")
						|| path.EndsWith("/system.xml.linq.dll")
						|| path.EndsWith("/system.xml.dll"))
					{
						standardRefs.Add(assembly);
					}
					else if (path.EndsWith("/unityeditor.graphs.dll"))
					{
						editorRefs.Add(assembly);
					}
				}
	...
	public static UnityAssembly AssemblyIdFromAssetPath(string pathName)
	{
		...
		var isIgnoredScript = path.StartsWith("assets/webplayertemplates/");
		if (isIgnoredScript)
			return UnityAssembly.None;
		
		var isPlugins = path.StartsWith("assets/plugins/");
		var isStandardAssets = path.StartsWith("assets/standard assets/") || path.StartsWith("assets/pro standard assets/");
...
public static class FGResolver
{
	...
	public static SymbolDefinition GetResolvedSymbol(ParseTree.BaseNode baseNode)
	{
		...
		throw new InvalidOperationException(node.RuleName);
	}
