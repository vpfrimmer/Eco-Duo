...
//		} catch {}
	...
	public bool TryEdit()
	{
		...
		//{
		//	EditorUtility.DisplayDialog("Script Inspector", "This asset is not open for edit!\n\n" + message, "OK");
		//}
		...
		if (!versionControlAsset.readOnly)
		{
#endif
			if (File.Exists(assetPath) && (File.GetAttributes(assetPath) & FileAttributes.ReadOnly) != 0)
				TryP4Checkout();
		...
		var checkoutTask = UnityEditor.VersionControl.Provider.Checkout(versionControlAsset, UnityEditor.VersionControl.CheckoutMode.Asset);
		try
		{
			checkoutTask.Wait();
	...
	public TextPosition GetOpenBraceLeftOf(int tokenLine, int tokenIndex)
	{
		TextPosition bracePosition = new TextPosition();
