...
#endif
{
	...
	private void OnGUI()
	{
		...
		switch (Event.current.type)
		{
			...
			case EventType.KeyDown:
				if (!tabSwitcher && Event.current.keyCode == KeyCode.Tab)
				{
					if (isOSX ?
						...
						!Event.current.alt && EditorGUI.actionKey)
					{
						...
						{
							tabSwitcher = TabSwitcher.Create(!isShift);
						};
					}
				}
				...
				if (DragAndDrop.objectReferences.Length > 0)
				{
					...
					foreach (Object obj in DragAndDrop.objectReferences)
					{
						if (AssetDatabase.GetAssetPath(obj).EndsWith(".dll", System.StringComparison.OrdinalIgnoreCase) ||
							AssetDatabase.GetAssetPath(obj).EndsWith(".exe", System.StringComparison.OrdinalIgnoreCase))
								continue;
...
#else
		if (titleContent.text.StartsWith("*"))
		{
			if (!isModified)
	...
	public void AddItemsToMenu(GenericMenu menu)
	{
		if (!string.IsNullOrEmpty(textEditor.targetPath))
		{
			...
			{
				menu.AddItem(new GUIContent("Close Tab _%w"), false, () => {
					Close();
				});
				...
				menu.AddItem(new GUIContent("Close Other SI Tabs _#%w"), false, () => {
					var allWindows = new FGCodeWindow[codeWindows.Count];
					codeWindows.CopyTo(allWindows);
					foreach (var window in allWindows)
...
#if UNITY_EDITOR_OSX
	[MenuItem("Window/Toggle Si3 Tabs #%t")] // shift-command-T
#else
	[MenuItem("Window/Toggle Si3 Tabs &t")] // Alt+T
#endif
