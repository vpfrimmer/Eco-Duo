...
//*
		...
		parser.Add(new Rule("memberInitializer",
			(IDENTIFIER | ".MEMBERINITIALIZER") - "=" - (expression | objectOrCollectionInitializer)
			));
		...
		parser.Add(new Rule("inclusiveOrExpression",
			exclusiveOrExpression - new Many("|" - exclusiveOrExpression)
			) { autoExclude = true });
		...
		parser.Add(new Rule("andExpression",
			equalityExpression - new Many("&" - equalityExpression)
			) { autoExclude = true });

		parser.Add(new Rule("equalityExpression",
			relationalExpression - new Many( (new Lit("==") | "!=") - relationalExpression )
			) { autoExclude = true });

		parser.Add(new Rule("relationalExpression",
			new Seq( shiftExpression, new Many(
				new Seq( new Lit("<") | ">" | "<=" | ">=", shiftExpression )
...
/*
		parser.Add(new Rule("primaryNoArrayCreationExpression",
		    LITERAL
			| simpleName
