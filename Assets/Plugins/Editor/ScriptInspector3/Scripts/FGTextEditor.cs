...
public class FGTextEditor
{
	...
	private static Theme[] themes = {
		...
		new Theme {
			...
			constants               = new Color32(0x4d, 0x16, 0xd8, 0xff),
			strings					= new Color32(0xce, 0x2f, 0x30, 0xff),
			builtInLiterals         = new Color32(0x65, 0x3c, 0x98, 0xff),
			operators               = Color.black,
		
			referenceTypes			= new Color32(0x80, 0x46, 0xb0, 0xff),
			valueTypes				= new Color32(0x80, 0x46, 0xb0, 0xff),
			interfaceTypes			= new Color32(0x80, 0x46, 0xb0, 0xff),
			enumTypes				= new Color32(0x80, 0x46, 0xb0, 0xff),
			delegateTypes			= new Color32(0x80, 0x46, 0xb0, 0xff),
			builtInTypes			= new Color32(0x80, 0x46, 0xb0, 0xff),
		
			namespaces              = Color.black,
			methods                 = new Color32(0x65, 0x3c, 0x98, 0xff),
			fields                  = Color.black,
			properties              = Color.black,
			events                  = Color.black,
			...
			enumMembers             = new Color32(0x4d, 0x16, 0xd8, 0xff),
		
			preprocessor            = new Color32(0x77, 0x4b, 0x31, 0xff),
			defineSymbols           = new Color32(0x77, 0x4b, 0x31, 0xff),
			...
			searchResults			= new Color32(255, 255, 0, 255),
		...
		new Theme {
			...
			lineNumbersHighlight = new Color32(43, 145, 175, 255),
...
//	private float contentWidth = 0;
	...
	[NonSerialized]
	private bool expandSnippets = true;
...
//			}
	...
	public void OnUpdate()
	{
		...
		{
			Debug.Log(showArgumentsHintForLeaf);
		}
	...
	private bool tryArgumentsHint = false;
...
//				parentWindow.Focus();
	...
	private void UpdateMatchingBraces()
	{
		...
		var indexLeft = index;
		var skipOver = 0;
		matchingBraceLeft = textBuffer.GetOpenBraceLeftOf(lineLeft, indexLeft);
		//while (indexLeft < 0)
		//{
		//	if (--lineLeft < 0)
		//		break;
		//	tokens = formatedLines[lineLeft].tokens;
		//	indexLeft = tokens.Count - 1;
		//}
		//while (indexLeft >= 0)
		//{
		//	tokenLeft = tokens[indexLeft];
		...
		//			else
		//			{
		//				matchingBraceLeft = new TextPosition(lineLeft, indexLeft);
		//				break;
		//			}
		//		}
		//		else if (')' == c || ']' == c || '}' == c)
		...
		//			break;
		//		tokens = formatedLines[lineLeft].tokens;
		//		indexLeft = tokens.Count;
		//	}
		//}
		
		var lineRight = line;
		var indexRight = index;
		SyntaxToken tokenRight;	
		tokens = formatedLines[lineRight].tokens;
		if (tokens == null)
			return;
		skipOver = 0;
		var numTokens = tokens.Count;
		while (lineRight < formatedLines.Length)
		{
			while (++indexRight >= numTokens)
			{
			...
			if (indexRight >= numTokens)
				break;
			tokenRight = tokens[indexRight];
			...
			if (tokenRight.tokenKind == SyntaxToken.Kind.Punctuator && text.Length == 1)
			{
				...
				if (')' == c || ']' == c || '}' == c)
				{
					...
					{
						matchingBraceRight = new TextPosition(lineRight, indexRight);
						break;
					}
				}
				else if ('(' == c || '[' == c || '{' == c)
	...
	public void DoGUI(bool enableGUI)
	{
		...
		if (hasCodeViewFocus)
		{
			...
			if (autocompleteWindow != null)
			{
				...
				if (FGListPopup.shortAttributeNames && committedWord != null && committedWord.EndsWith("Attribute"))
				...
				if (textBuffer.isCsFile)
				{
					...
					if (token != null)
					{
						...
						if (helpUrl != null)
						{
							...
							else if (helpUrl.StartsWith("file:///unity/ScriptReference/"))
							{
						...
						if (token.parent != null && token.parent.resolvedSymbol != null)
						{
							...
							if (assembly != null)
							{
								...
								{
									codeViewPopupMenu.AddItem(new GUIContent(isOSX ? "Go to Definition %y" : "Go to Definition _f12"), false, GoToDefinition);
								}
							}
...
//				contentRect.xMax += marginLeft;
	...
	private GUIStyle GetTokenStyle(SyntaxToken token)
	{
		...
		if (token.tokenKind == SyntaxToken.Kind.ContextualKeyword)
		{
			...
			if (token.text == "var" && token.parent != null && token.parent.parent != null && token.parent.parent.resolvedSymbol == null)
		...
		if (leaf != null && leaf.parent != null)
		{
			if (token.tokenKind == SyntaxToken.Kind.Keyword)
			{
				if ((token.text == "base" || token.text == "this") && leaf.resolvedSymbol == null)
		...
		if (highlightedSymbol is InstanceDefinition)
		{
			...
			case SymbolKind.CatchParameter:
				...
				else if (parentRule == "accessIdentifier" && parent.parent.RuleName == "qidPart")
				{
					...
					parentRule == "accessIdentifier" && parent.parent.RuleName == "primaryExpressionPart")
				{
...
//					return;
	...
	private void IndentMoreOrInsertTab()
	{
		...
		{
			if (expandSnippets && ExpandSnippet())
				return;
		...
		if (autocompleteWindow == null)
		{
			...
			//	Debug.Log("CurrentParseTreeNode.declaration: " + scanner.CurrentParseTreeNode.declaration);
				...
				//	Debug.Log(completionTypes);
					...
					data.RemoveWhere(x => !x.IsValid() || x.name.StartsWith("<"));
			...
			|| assembly.assemblyId == AssemblyDefinition.UnityAssembly.CSharpEditor)
		{
			...
			if (declarations != null && declarations.Count > 0)
			{
				foreach (var decl in declarations)
					if (decl.IsValid() && GoToSymbolDeclaration(decl))
						break;
			}
			else
			{
				Debug.Log(symbol.GetTooltipText());
			}
		}
	}
	...
	private void ProcessEditorKeyboard(Event current, bool acceptingAutoComplete)
	{
			...
			&& (isOSX ? !current.control : !current.command))
		{
				...
				!isOSX && !current.control && current.keyCode == KeyCode.F1)
			{
				...
				if (helpUrl != null)
				{
					...
					if (helpUrl.StartsWith("file:///unity/ScriptReference/"))
						Help.ShowHelpPage(helpUrl);
					else
						Help.BrowseURL(helpUrl);
				}
				...
				? current.command && current.keyCode == KeyCode.Y
				: !current.control && current.keyCode == KeyCode.F12)
			{
				current.Use();
		...
		if (current.type == EventType.keyDown)
		{
				...
				&& TryEdit())
			{
				...
				if (selectionStartPosition == null && "{[(\"<".IndexOf(typedChar) != -1)
				{
				...
				if (text.Length == 1 && autocompleteWindow == null)
				{
					...
					if (tokenLeft != null && tokenLeft.tokenKind == SyntaxToken.Kind.Punctuator &&
						tokenLeft.parent != null && tokenLeft.parent.syntaxError == null)
					{
						if (text == ".")
						{
...
//				nextCharacterIndex = textBuffer.ColumnToCharIndex(ref nextCaretColumn, nextCaretLine);
	...
	private void DrawPing(float indent, Rect rcPing, bool bgOnly)
	{
		...
		if (t > 0f && t < 64f)
		{
			...
			{
				if (!bgOnly)
					GUI.backgroundColor = new Color(pingColor.r, pingColor.g, pingColor.b, (8f - t) * 0.125f);
				else
					GUI.backgroundColor = new Color(pingColor.r, pingColor.g, pingColor.b);
				//else
				//	GUI.backgroundColor = new Color(pingColor.r, pingColor.g, pingColor.b, (64f - t) * 0.125f);
				if (t > 56f)
					GUI.color = new Color(oldColor.r, oldColor.g, oldColor.b, (64f - t) * 0.125f);
			}
			else
			{
				GUI.backgroundColor = new Color(pingColor.r, pingColor.g, pingColor.b);				
				...
				else
				{
					...
					for (int index = 0; index < p4MenusMethods.Length; ++index)
					{
						...
						foreach (MenuItem menuItem in methodInfo.GetCustomAttributes(typeof(MenuItem), false))
						{
							...
							if (!menuItem.menuItem.StartsWith("Assets/Perforce/"))
								continue;
	...
	public void PingText(FGTextBuffer.CaretPos startPosition, int numChars, Color color)
	{
		...
		int toCharIndex = selectionStartPosition.characterIndex + numChars;
	...
	private static void SetCodeStyleVSDarkWithVAX() { SelectTheme(10, false); }
	...
	[MenuItem("Help/About Script Inspector...", false, 1000)]
